...
void Camera::Refresh3p()
{
	...
	glLoadIdentity();
	gluLookAt(m_x + 2*m_lx, m_y + 2*m_ly, m_z + 2*m_lz, m_x, m_y, m_z, 0.0,1.0,0.0);
...
void Camera::Strafe3p(float incr)
{
	m_x = m_x - incr*m_strafe_lx;
	m_z = m_z - incr*m_strafe_lz;
...
void Camera::RotatePitch3p(float angle)
{
    const float limit = 89.0 * M_PI / 180.0;
